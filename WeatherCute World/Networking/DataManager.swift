...
struct DataManager<T: SearchType> {
    ...
    private static func fetchArray(url: URL, completion: @escaping (Result<[LocationData]>) -> Void) {
        Networker.fetchData(url: url) { result in
            switch result {
            case .success(let data):
                guard let response = try? JSONDecoder.weatherApiDecoder.decode([LocationData].self, from: data) else {
                    return
                }
                completion(.success(response))
            case .failure(let error):
    ...
    static func fetchArray(completion: @escaping (Result<[LocationData]>) -> Void) {
        fetchArray(url: T.endpoint.url()) { result in
            switch result {
            case .success(let result):
